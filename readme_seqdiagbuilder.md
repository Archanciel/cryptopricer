# SeqdiagBuilder
Builds a UML sequence diagram from Python source code.
![](screenshots/CryptoPricerWebp.net-gifmaker.gif)


## What does it do ?
SeqDiagBuilder creates a Plantuml command file and then runs Plantuml on it to create a svg sequence diagram file which can be viewed in a browser.

## How does it work ?
SeqDiagBuilder fills its goal in 2 steps:
* first, it records the control flow (the stack of calls) during the execution of the entry call.
* then, on demand, it creates the command file from the recorded info in step one, calls Plantuml on it and open a browser on the sequence diagram svg file.

## How is the execution flow recorded ?
The principle is simple: add the following 2 lines in every leaf method which must be  included in the sequence diagram:
        from seqdiagbuilder import SeqDiagBuilder
        SeqDiagBuilder.recordFlow()

For example, if you have classA.foo() -> classB.bar() -> classC.baz(), the call to the recordFlow() method is put somewhere in classC.baz().

The recordFlow method does nothing (i.e. returns immediately) if SeqDiagBuilder has not been activated prior to the execution of the code to document.

A detailed usage example is proposed below.
