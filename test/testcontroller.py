	...
	def testControllerInvalidYearThenValidDDMMInFuture(self):
		...
		try:
			with open(FILE_PATH, 'w') as outFile:
				sys.stdout = outFile
				self.controller.commandLineLoop() #will eat up what has been filled in stdin using StringIO above
		except:
			pass
...
#            self.assertEqual('ETH/BTC on Bitfinex: ' + '{}/{}/{} {}:{}R'.format(nowDayStr, nowMonthStr, nowYearStr, nowHourStr, nowMinuteStr), UtilityForTest.removeOneEndPriceFromResult(contentList[7][:-1])) #removing \n from contentList entry !
	...
	def testControllerBugSpecifyInvalTimeAfterAskedRT345(self):
		...
		try:
			with open(FILE_PATH, 'w') as outFile:
				sys.stdout = outFile
				self.controller.commandLineLoop() #will eat up what has been filled in stdin using StringIO above #will eat up what has been filled in stdin using StringIO above
		except:
			pass
